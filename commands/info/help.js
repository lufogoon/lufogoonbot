...
module.exports = {
    ...
    aliases: ["h"],
    category: "info",
    description: "vous donne de l'aide sur une commande ou affiche la liste des commandes",
    usage: "[command | alias]",
...
function getCMD(bot, message, input) {
    ...
    let info = `Aucune information pour cette commande **${input.toLowerCase()}**`;

    if (!cmd) {
        return message.channel.send(embed.setColor("RANDOM").setDescription(info));
    }

    if (cmd.name) info = `**Nom de commande**: ${cmd.name}`;
    if (cmd.aliases) info += `\n**Aliases**: ${cmd.aliases.map(a => `\`${a}\``).join(", ")}`;
    if (cmd.description) info += `\n**Description**: ${cmd.description}`;
    if (cmd.usage) {
        info += `\n**Utilisation**: ${cmd.usage}`;
        embed.setFooter(`Syntax: <> = Obligatoire, [] = Option`);
    }
